...
{
	if(m_video_device.empty() && m_audio_device.empty()) return false;
	...
	if(!m_audio_device.empty())
	{
		...
		if (avformat_open_input(&m_pAudFmtCtx, device_name_utf8.c_str(), m_pInputFormat, &device_param) != 0) return false;

		//input audio initialize
		if (avformat_find_stream_info(m_pAudFmtCtx, NULL) < 0) return false;

		m_audioindex = -1;
		for (i = 0; i <(int)m_pAudFmtCtx->nb_streams; i++)
		{
		...
		if (m_audioindex == -1) return false;

		if (avcodec_open2(m_pAudFmtCtx->streams[m_audioindex]->codec, avcodec_find_decoder(m_pAudFmtCtx->streams[m_audioindex]->codec->codec_id), NULL) < 0) return false;
...
void  CAVInputStream::CloseInputStream()
{
	...
	{  
		WaitForSingleObject(m_hCapVideoThread, 3000);
	...
	{  
		WaitForSingleObject(m_hCapAudioThread, 3000);
...
int CAVInputStream::ReadAudioPackets()
{
	...
	while (encode_audio)
	{
		...
		* to flush it.
		*/
		if ((ret = avcodec_decode_audio4(m_pAudFmtCtx->streams[m_audioindex]->codec, input_frame, &dec_got_frame_a, &input_packet)) < 0) return ret;
