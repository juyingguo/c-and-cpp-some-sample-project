...
bool CAVOutputStream::OpenOutputStream(const char* out_path)
{
	...
	if(m_video_codec_id != 0)
	{
		...
		if (avcodec_open2(pCodecCtx, pCodec, &param) < 0) return false;


		//Add a new stream to output,should be called by the user before avformat_write_header() for muxing
		video_st = avformat_new_stream(ofmt_ctx, pCodec);
		if (video_st == NULL) return false;
	...
	if(m_audio_codec_id != 0)
	{
		...
		if (avcodec_open2(pCodecCtx_a, pCodec_a, NULL) < 0) return false;


		//Add a new stream to output,should be called by the user before avformat_write_header() for muxing
		audio_st = avformat_new_stream(ofmt_ctx, pCodec_a);
		if (audio_st == NULL) return false;
		...
		*/
		if (!(m_converted_input_samples = (uint8_t**)calloc(pCodecCtx_a->channels, sizeof(**m_converted_input_samples)))) return false;

		m_converted_input_samples[0] = NULL;
	}

	//Open output URL,set before avformat_write_header() for muxing
	if (avio_open(&ofmt_ctx->pb, out_path, AVIO_FLAG_READ_WRITE) < 0) return false;
...
int  CAVOutputStream::write_audio_frame(AVStream *input_st, AVFrame *input_frame, INT64 lTimeStamp)
{
	...
	if(m_first_aud_time == -1) m_first_aud_time = lTimeStamp;
	...
	*/
		...
		*/
	{
		...
		*/
		if (av_audio_fifo_read(m_fifo, (void **)output_frame->data, frame_size) < frame_size) return AVERROR_EXIT;
